...
export default function BookingPage() {
  ...
  const slot = parkingSlots.find((item) => item.id === searchParams.get('slotId')) || parkingSlots.find((item) => item.status === 'Available')
  const site = parkingSites.find((item) => item.id === slot.siteId)
  ...
  const afterDeduction = user.walletBalance - fee
  const insufficient = afterDeduction < 0

  const requestConfirmation = (event) => {
    event.preventDefault()
    const id = `BK-${Math.floor(1000 + Math.random() * 9000)}`
    setPendingBooking({ id, qrCode: id, userId: user.id, userName: user.name, vehiclePlate: form.vehiclePlate, siteId: site.id, siteName: site.name, slotId: slot.id, slotNumber: slot.slotNumber || slot.id.replace('SLOT-', ''), date: form.date, startTime: form.time, duration: `${form.duration}h`, durationHours: Number(form.duration), estimatedFee: fee, status: 'Reserved' })
  }
